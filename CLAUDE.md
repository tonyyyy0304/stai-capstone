...
- **CV/DS domain integration (mandatory component):** OCR/document field extraction via Gemini 2.5 Flash multimodal input + `response_schema` — see PLAN.md §5 for why this was chosen over a standalone OCR engine.
...
- Document validation rules (identity match, validity window, extraction-confidence floor) are deterministic code in `src/guardrails/`, not LLM judgment — see PLAN.md §4.1. Fail toward "needs human review."
