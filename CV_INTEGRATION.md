...
| --- | --- | --- |
| Raw image bytes | Never persisted past the request (`PERSIST_UPLOADS = False`); `load_image()` decodes via PIL (`ImageOps.exif_transpose()` corrects rotation for phone photos, `2026-08-10`) then converts straight to a numpy array, which strips all EXIF — including geolocation — as a side effect regardless of what the source file had | §2.2, §2.4 |
...
**Quota discipline during initial verification:** `run_validation_eval.py` runs entirely off `extractor.load_cached_result()` (hash-only lookup, never calls the vision model) plus a purely local `quality.assess()` re-run — genuinely zero API cost, not just documented as such. `run_ocr_eval.py` does call the model on a cache miss, so the first verification pass was deliberately scoped with `--limit 1`. Once the user switched to a paid Gemini API key (2026-08-10), quota caution stopped applying and both scripts were re-run at full scale.

**Full-scale results, 2026-08-10 (paid key, all 16 mock identities + 26 negatives):**
- `run_ocr_eval.py --subset mock`: **100% exact-match** on every field, both doc types, all 16 identities. `id_type` classification 100%.
- `run_ocr_eval.py --subset mock --no-preprocess`: **97.2%** overall for NBI (down from 100%) — `date_printed` specifically drops to 75% exact-match (avg CER 0.118). A genuinely different, measurable ablation effect, not just mechanically separate cache entries (the earlier `--limit 1` smoke run happened to tie at 100%/100% since that one fixture was easy either way).
- `run_validation_eval.py`: NBI 18/18 fixtures evaluated, 100% precision/recall/F1, **0% false auto-pass rate**, `validity_window` correctly triggering on the expired negatives (2/16). Government ID 17/18 evaluated (1 fixture — `id_neg_cross_name_mismatch_1.png` — hits a real, reproducible intermittent hang, see below; not yet re-attempted after that fix), same 100%/100%/1.000 and 0% false auto-pass. Cross-document check: 11/12 evaluated, **27.3% overall mismatch-trigger rate**, **0% false-trigger rate on clean pairs** — correctly silent on genuinely matching documents, correctly fires on genuinely mismatched ones.

**A real, reproducible hang found while extracting negative-fixture coverage — not a fluke, isolated to a root cause.** `extract_document()` on specific images (confirmed on `id_neg_cross_name_mismatch_1.png`, reproducible across multiple attempts) hung **indefinitely** — no exception, no timeout, just never returned. Isolated by timing every internal step by hand: quality gate (instant, local), `preprocess()` (instant, local), prompt construction (instant) all completed in ~1s; the hang was specifically in the network call to Gemini, and specifically when sending the **preprocessed/re-encoded** bytes (a direct call with the original, unprocessed bytes on the same image succeeded in seconds). Root cause: **`config.get_gemini_client()` had no request timeout configured anywhere** — `google.genai.types.HttpOptions.timeout` (milliseconds) was simply never set, so a stalled request had nothing to bound it. In a FastAPI context this means a single stalled `/upload-doc` request could hang a worker indefinitely.

Fixed: `config.GEMINI_REQUEST_TIMEOUT_MS = 120_000`, passed via `http_options=genai.types.HttpOptions(timeout=...)` in `get_gemini_client()`. **This surfaced a second, related gap**: a timeout raises `httpx.ConnectTimeout`/`ReadTimeout`, which is **not** a `google.genai.errors.APIError` subclass — every `except (APIError, LLMBackendError)` clause in the codebase (`extractor.py`, `orchestrator.py`, `llm_judge.py`) would have let a timeout propagate as an unhandled crash instead of degrading gracefully. Added `httpx.TimeoutException` to all three, plus `embeddings.py`'s retry loop (where a timeout is treated exactly as retryable as a 429, same backoff). Re-ran the previously-hanging call after the fix: it failed gracefully in bounded time (`(None, report)`, logged `ocr_api_error status=?`) instead of hanging — real proof the fix works, not just a passing unit test. 6 new tests across `test_extractor.py`/`test_embeddings.py`/`test_orchestrator.py`.

**Acceptance criteria:**
- [x] Per-field exact-match + normalized CER printed to console and written to `evals/results/`, broken out per `doc_type` — verified at full scale.
- [x] `--no-preprocess` produces a **distinct number** from the default run — verified at full scale (97.2% vs 100.0% for NBI, `date_printed` specifically).
- [x] `run_validation_eval.py` reports precision/recall/F1 and the false auto-pass rate per `doc_type`, plus per-rule trigger counts, plus a dedicated `cross_document_consistency` section — verified at near-full scale (17-18 of 18 fixtures per doc type), and it's what caught the real `wrong_person` bug above.
- [x] `--subset real`/`--subset mock` never pooled — verified (separate report files, separate JSON sections).
- [x] A second run against the same cache spends 0 API calls — verified by construction for `run_validation_eval.py`; verified for `run_ocr_eval.py` via distinct cache files per preprocess setting.
