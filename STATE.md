...
- **UI** (`src/ui.py`) — sidebar is checklist-only (Employee ID field + Document Checklist card, color-coded status pills, refetched every render, header shows which employee it's for plus an "N of 2 validated" progress line). The upload flow lives in the **main chat column**, positioned right above the composer (a fixed spot regardless of conversation length — earlier attempts placed it above the message list, or right after it, both of which looked unchanged/wrong on a short conversation), wrapped in a `st.spinner()` restyled to the accent color during the `/upload-doc` call, auto-expanding the first time it's revealed (respects manual toggling after that via its own `key`). Doc-type dropdown now derives from `config.REQUIRED_ONBOARDING_DOCS` instead of a hardcoded list. DOB gets real calendar validation (`date.fromisoformat`, catches `2023-13-40`-shaped garbage) with its own inline error before the network call. Selected file shows a thumbnail preview before submit. Result banner deferred to the next render (session_state + `st.rerun()`) so it survives that rerun, which is also what refreshes the checklist without a manual reload.
...
- **Conversationally-gated document flow** (`src/agent/orchestrator.py`, `src/ui.py`) — the Employee ID field, checklist, and uploader are now hidden entirely until the conversation actually calls for document verification, rather than sitting there with no context on every page load. `AgentResponse` gained an `actions: list[dict]` field (dict-shaped to match `api.py`'s `ActionResponse` without importing that Pydantic model into the orchestrator); `Intent.DOCUMENT_UPLOAD`/`DOCUMENT_STATUS` populate it with `{"type": "unlock_document_flow", ...}`. `src/ui.py` watches for that action type on each new assistant message and flips `st.session_state.show_upload_flow = True` — persists for the rest of the session once unlocked (a later unrelated question doesn't re-hide it), resets on "New chat". Closes the loop on `handle_message()`'s `actions` field, which had been hardcoded to `[]` since Phase 6.
